...
class Assign(models.Model):
        ...
        def __str__(self):
            ...
            return '%s : %s : %s' % (cr.shortname,cl,tr.name)
...
class AttendenceTotal(models.Model):
        student=models.ForeignKey(Student,on_delete=models.CASCADE)
        course=models.ForeignKey(Course,on_delete=models.CASCADE)
        class Meta:
        ...
        def att_class(self):
            ...
            cr_id=Course.objects.get(name=self.course)
        ...
        def total_class(self):
            stud=Student.objects.get(name=self.student)
            cr_id=Course.objects.get(name=self.course)
            total_class=Attendence.objects.filter(student=stud,course=cr_id).count()
            return total_class
        @property
        def attendence(self):
            stud=Student.objects.get(name=self.student)
